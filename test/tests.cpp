...
static void number_format_tests(void)
{
	...
	ra(php::number_format(1337, 3) == "1,337.000");
...
static void rawurlencode_tests(void)
{
	{
		const std::string q = "hi";		//BIN_STRING("\xFF\x00\xFFlol");
		const std::string q_answer = q; //"%ff%00%fflol";
	...
	{
		const std::string q = BIN_STRING("\x00\xFF\x00hi ");
		const std::string q_answer = BIN_STRING("%00%FF%00hi%20");
		/*dump_string(q);
		dump_string(php::rawurlencode(q_answer));
		dump_string(q_answer);
		*/
		ra(php::rawurlencode(q) == q_answer);
	}
	{
		...
		//dump_string(php::rawurlencode(q));
		//dump_string(q_answer);
		ra(php::rawurlencode(q) == q_answer);
	}
}
...
{
	run();
	std::cout << "all tests ran successfully! :D" << std::endl;
	return 0;
}